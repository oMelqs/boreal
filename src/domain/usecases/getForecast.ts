...
/**
 * Previsão horária de hoje e amanhã para a cidade, no timezone dela. Ponto
 * único de passagem entre a UI e o port — os hooks nunca tocam o repository
 * direto.
 */
export function getForecast(repository: WeatherRepository, city: City): Promise<HourlyForecast[]> {
  return repository.getHourlyForecast(city);
}
